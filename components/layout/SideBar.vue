...
const mainNavigations = computed(() =>
  options.routes.filter(({ path }) => [ '/', '/about', '/packages' ].includes(path))
    .sort(({ path: firstPath }, { path: secondPath }) => firstPath < secondPath ? -1 : 1)
)
...
<template>
  <aside class="fixed md:sticky top-0 left-0 flex-shrink-0 w-screen md:w-56 h-full max-sm:px-4 max-sm:pt-20 pb-6 bg-background overflow-y-auto z-50" :class="{'max-md:hidden': !open}">
